...
namespace IOData {
	...
	/** コンストラクタ */
	IODataSequentialLayerGPU_base::IODataSequentialLayerGPU_base(Gravisbell::GUID guid, Gravisbell::IODataStruct ioDataStruct)
		:	guid				(guid)
	...
	/** デストラクタ */
	IODataSequentialLayerGPU_base::~IODataSequentialLayerGPU_base()
	{
	...
	/** 初期化. 各ニューロンの値をランダムに初期化
		@return	成功した場合0 */
	ErrorCode IODataSequentialLayerGPU_base::Initialize(void)
	{
	...
	//==============================
	/** レイヤー種別の取得 */
	U32 IODataSequentialLayerGPU_base::GetLayerKind()const
	{
	...
	/** レイヤー固有のGUIDを取得する */
	Gravisbell::GUID IODataSequentialLayerGPU_base::GetGUID(void)const
	{
		return this->guid;
	}
	...
	/** レイヤー種別識別コードを取得する.
		...
		@return 成功した場合0 */
	Gravisbell::GUID IODataSequentialLayerGPU_base::GetLayerCode(void)const
	{
	...
	/** レイヤーの設定情報を取得する */
	const SettingData::Standard::IData* IODataSequentialLayerGPU_base::GetLayerStructure()const
	{
		return NULL;
	}
	...
	//==============================
	/** データの構造情報を取得する */
	IODataStruct IODataSequentialLayerGPU_base::GetDataStruct()const
	{
	...
	U32 IODataSequentialLayerGPU_base::GetBufferCount()const
	{
	...
	Gravisbell::ErrorCode IODataSequentialLayerGPU_base::PreProcessLearn(U32 batchSize)
	{
		// 通常の演算用の処理を実行
	...
	Gravisbell::ErrorCode IODataSequentialLayerGPU_base::PreProcessCalculate(U32 batchSize)
	{
		// バッチサイズの保存
	...
	ErrorCode IODataSequentialLayerGPU_base::PreProcessLoop()
	{
		...
		cudaMemset(thrust::raw_pointer_cast(&this->lpErrorValue_ave[0]), 0, sizeof(F32)*this->lpErrorValue_ave.size());
		cudaMemset(thrust::raw_pointer_cast(&this->lpErrorValue_ave2[0]), 0, sizeof(F32)*this->lpErrorValue_ave2.size());
		cudaMemset(thrust::raw_pointer_cast(&this->lpErrorValue_crossEntropy[0]), 0, sizeof(F32)*this->lpErrorValue_crossEntropy.size());
	...
	U32 IODataSequentialLayerGPU_base::GetBatchSize()const
	{
	...
	Gravisbell::ErrorCode IODataSequentialLayerGPU_base::CalculateLearnError(Gravisbell::CONST_BATCH_BUFFER_POINTER i_lppInputBuffer)
	{
	...
	/** 誤差の値を取得する.
		...
		@param	o_ave	平均誤差.
		@param	o_ave2	平均二乗誤差. */
	ErrorCode IODataSequentialLayerGPU_base::GetCalculateErrorValue(F32& o_max, F32& o_ave, F32& o_ave2, F32& o_crossEntropy)
	{
		o_max  = 0.0f;
		o_ave  = 0.0f;
		o_ave2 = 0.0f;
		...
		// max
		cudaMemcpy(&lpTmporaryBufer[0], thrust::raw_pointer_cast(&this->lpErrorValue_max[0]), sizeof(F32)*this->GetBufferCount(), cudaMemcpyDeviceToHost);
		for(U32 inputNum=0; inputNum<this->GetBufferCount(); inputNum++)
			o_max   = max(o_max, lpTmporaryBufer[inputNum]);

		// ave
		cudaMemcpy(&lpTmporaryBufer[0], thrust::raw_pointer_cast(&this->lpErrorValue_ave[0]), sizeof(F32)*this->GetBufferCount(), cudaMemcpyDeviceToHost);
		for(U32 inputNum=0; inputNum<this->GetBufferCount(); inputNum++)
			o_ave   += lpTmporaryBufer[inputNum];
		o_ave  = o_ave / this->calcErrorCount / this->GetBufferCount();

		// ave2
		cudaMemcpy(&lpTmporaryBufer[0], thrust::raw_pointer_cast(&this->lpErrorValue_ave2[0]), sizeof(F32)*this->GetBufferCount(), cudaMemcpyDeviceToHost);
		for(U32 inputNum=0; inputNum<this->GetBufferCount(); inputNum++)
			o_ave2   += lpTmporaryBufer[inputNum];
		o_ave2 = (F32)sqrt(o_ave2 / this->calcErrorCount / this->GetBufferCount());

		// crossEntropy
		cudaMemcpy(&lpTmporaryBufer[0], thrust::raw_pointer_cast(&this->lpErrorValue_crossEntropy[0]), sizeof(F32)*this->GetBufferCount(), cudaMemcpyDeviceToHost);
		for(U32 inputNum=0; inputNum<this->GetBufferCount(); inputNum++)
	...
	ErrorCode IODataSequentialLayerGPU_base::GetCalculateErrorValueDetail(F32 o_lpMax[], F32 o_lpAve[], F32 o_lpAve2[])
	{
		for(U32 inputNum=0; inputNum<this->GetBufferCount(); inputNum++)
		{
	...
	/** 入力データ構造を取得する.
		@return	入力データ構造 */
	IODataStruct IODataSequentialLayerGPU_base::GetInputDataStruct()const
	{
		return this->GetDataStruct();
	}

	/** 入力バッファ数を取得する. byte数では無くデータの数なので注意 */
	U32 IODataSequentialLayerGPU_base::GetInputBufferCount()const
	{
	...
	CONST_BATCH_BUFFER_POINTER IODataSequentialLayerGPU_base::GetDInputBuffer()const
	{
	...
	Gravisbell::ErrorCode IODataSequentialLayerGPU_base::GetDInputBuffer(BATCH_BUFFER_POINTER o_lpDInputBuffer)const
	{
	...
	//==============================
	/** 出力データ構造を取得する */
	IODataStruct IODataSequentialLayerGPU_base::GetOutputDataStruct()const
	{
		return this->GetDataStruct();
	}

	/** 出力バッファ数を取得する. byte数では無くデータの数なので注意 */
	U32 IODataSequentialLayerGPU_base::GetOutputBufferCount()const
	{
		return this->GetBufferCount();
	}
	...
	CONST_BATCH_BUFFER_POINTER IODataSequentialLayerGPU_base::GetOutputBuffer()const
	{
	...
	/** 出力データバッファを取得する.
		...
		@return 成功した場合0 */
	Gravisbell::ErrorCode IODataSequentialLayerGPU_base::GetOutputBuffer(BATCH_BUFFER_POINTER o_lpOutputBuffer)const
	{
