// IODataSequentialLayer.cpp : DLL アプリケーション用にエクスポートされる関数を定義します。
//
...
namespace IOData {
	...
	class IODataSequentialLayerCPU : public IIODataSequentialLayer
	{
	private:
	...
	public:
		/** コンストラクタ */
		IODataSequentialLayerCPU(Gravisbell::GUID guid, Gravisbell::IODataStruct ioDataStruct)
			:	guid				(guid)
		...
		/** デストラクタ */
		virtual ~IODataSequentialLayerCPU()
		{
		}
	...
	public:
		...
		Gravisbell::ErrorCode SetData(U32 i_dataNo, const F32 lpData[])
		{
			...
			// コピー
			memcpy(lpBatchDataPointer[i_dataNo], lpData, sizeof(F32)*this->GetBufferCount());
		...
		{
			return (U32)this->lpBatchDataPointer.size();
		}
		/** データを番号指定で取得する.
			@param num		取得する番号
		...
		{
			if(num >= this->lpBatchDataPointer.size())
				return ErrorCode::ERROR_CODE_COMMON_OUT_OF_ARRAYRANGE;
			...
			{
				o_lpBufferList[i] = this->lpBatchDataPointer[num][i];
			}
...
//						this->lpDInputBuffer[batchNum][inputNum] = -(output - teach) / (output * (1.0f - output));
	...
	extern IODataSequentialLayer_API Gravisbell::Layer::IOData::IIODataSequentialLayer* CreateIODataSequentialLayerCPU(Gravisbell::IODataStruct ioDataStruct)
	{
		...
		return CreateIODataSequentialLayerCPUwithGUID(uuid.data, ioDataStruct);
	}
	...
	{
		return new Gravisbell::Layer::IOData::IODataSequentialLayerCPU(guid, ioDataStruct);
	}
