...
namespace NeuralNetwork {
	...
	/** コンストラクタ */
	BatchExponentialNormalization_GPU::BatchExponentialNormalization_GPU(Gravisbell::GUID guid, BatchExponentialNormalization_LayerData_GPU& i_layerData, const IODataStruct& i_inputDataStruct, Gravisbell::Common::ITemporaryMemoryManager& i_temporaryMemoryManager)
	...
	/** デストラクタ */
	BatchExponentialNormalization_GPU::~BatchExponentialNormalization_GPU()
	{
	...
	//================================
	/** レイヤー種別の取得 */
	U32 BatchExponentialNormalization_GPU::GetLayerKind()const
	{
	...
	/** 初期化. 各ニューロンの値をランダムに初期化
		@return	成功した場合0 */
	ErrorCode BatchExponentialNormalization_GPU::Initialize(void)
	{
	...
	//===========================
	/** レイヤーデータを取得する */
	BatchExponentialNormalization_LayerData_Base& BatchExponentialNormalization_GPU::GetLayerData()
	{
		return this->layerData;
	}
	const BatchExponentialNormalization_LayerData_Base& BatchExponentialNormalization_GPU::GetLayerData()const
	{
		return this->layerData;
	}
	...
	ErrorCode BatchExponentialNormalization_GPU::PreProcessLearn()
	{
	...
	ErrorCode BatchExponentialNormalization_GPU::PreProcessCalculate()
	{
	...
	/** ループの初期化処理.データセットの実行開始前に実行する
		失敗した場合はCalculate以降の処理は実行不可. */
	ErrorCode BatchExponentialNormalization_GPU::PreProcessLoop()
	{
	...
	/** 演算処理を実行する.
		...
		@return 成功した場合0が返る */
	ErrorCode BatchExponentialNormalization_GPU::Calculate_device(CONST_BATCH_BUFFER_POINTER i_lppInputBuffer, BATCH_BUFFER_POINTER o_lppOutputBuffer)
	{
		cudnnStatus_t err_cudnn;
		...
		case ProcessType::PROCESSTYPE_LEARN:
			{
				...
				err_cudnn = cudnnBatchExponentialNormalizationForwardTraining(
					this->cudnnHandle,
		...
		case ProcessType::PROCESSTYPE_CALCULATE:
			{
				...
				err_cudnn = cudnnBatchExponentialNormalizationForwardInference(
					this->cudnnHandle,
					cudnnBatchNormMode_t::CUDNN_BATCHNORM_SPATIAL,
					&alpha,
					&beta,
	...
	/** 入力誤差計算をを実行する.学習せずに入力誤差を取得したい場合に使用する.
		...
		直前の計算結果を使用する */
	ErrorCode BatchExponentialNormalization_GPU::CalculateDInput_device(CONST_BATCH_BUFFER_POINTER i_lppInputBuffer, BATCH_BUFFER_POINTER o_lppDInputBuffer, CONST_BATCH_BUFFER_POINTER i_lppOutputBuffer, CONST_BATCH_BUFFER_POINTER i_lppDOutputBuffer)
	{
		cudnnStatus_t err_cudnn;
		...
		err_cudnn = cudnnBatchExponentialNormalizationBackward(
			this->cudnnHandle,
			cudnnBatchNormMode_t::CUDNN_BATCHNORM_SPATIAL,
			&alphaData,
			&betaData,
	...
	/** 学習処理を実行する.
		...
		@param	i_lppDOutputBuffer	出力誤差差分=次レイヤーの入力誤差差分.	[GetBatchSize()の戻り値][GetOutputBufferCount()の戻り値]の要素数が必要.
		直前の計算結果を使用する */
	ErrorCode BatchExponentialNormalization_GPU::Training_device(CONST_BATCH_BUFFER_POINTER i_lppInputBuffer, BATCH_BUFFER_POINTER o_lppDInputBuffer, CONST_BATCH_BUFFER_POINTER i_lppOutputBuffer, CONST_BATCH_BUFFER_POINTER i_lppDOutputBuffer)
	{
		cudnnStatus_t err_cudnn;
