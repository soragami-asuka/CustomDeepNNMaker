...
namespace NeuralNetwork {
	...
	//===========================
	/** コンストラクタ */
	ExponentialNormalization_LayerData_GPU::ExponentialNormalization_LayerData_GPU(const Gravisbell::GUID& guid)
		:	ExponentialNormalization_LayerData_Base(guid)
	{
	}
	/** デストラクタ */
	ExponentialNormalization_LayerData_GPU::~ExponentialNormalization_LayerData_GPU()
	{
	}
	...
	/** 初期化. 各ニューロンの値をランダムに初期化
		@return	成功した場合0 */
	ErrorCode ExponentialNormalization_LayerData_GPU::Initialize(void)
	{
	...
	/** 初期化. 各ニューロンの値をランダムに初期化
		...
		@return	成功した場合0 */
	ErrorCode ExponentialNormalization_LayerData_GPU::Initialize(const SettingData::Standard::IData& i_data)
	{
		ErrorCode err;
	...
	/** 初期化. バッファからデータを読み込む
		...
		@return	成功した場合0 */
	ErrorCode ExponentialNormalization_LayerData_GPU::InitializeFromBuffer(const BYTE* i_lpBuffer, U64 i_bufferSize, S64& o_useBufferSize )
	{
	...
	S64 ExponentialNormalization_LayerData_GPU::WriteToBuffer(BYTE* o_lpBuffer)const
	{
	...
	ILayerBase* ExponentialNormalization_LayerData_GPU::CreateLayer(const Gravisbell::GUID& guid, const IODataStruct i_lpInputDataStruct[], U32 i_inputLayerCount, Gravisbell::Common::ITemporaryMemoryManager& i_temporaryMemoryManager)
	{
		if(this->CheckCanUseInputDataStruct(i_lpInputDataStruct, i_inputLayerCount) == false)
			return NULL;

		return new CNNSingle2SingleLayerBase_GPU<ExponentialNormalization_GPU, ExponentialNormalization_LayerData_GPU>(guid, *this, i_lpInputDataStruct[0], i_temporaryMemoryManager);
	}
	...
	//===========================		
	/** オプティマイザーを変更する */
	ErrorCode ExponentialNormalization_LayerData_GPU::ChangeOptimizer(const wchar_t i_optimizerID[])
	{
...
{
	// 作成
	Gravisbell::Layer::NeuralNetwork::ExponentialNormalization_LayerData_GPU* pLayerData = new Gravisbell::Layer::NeuralNetwork::ExponentialNormalization_LayerData_GPU(guid);
	if(pLayerData == NULL)
		return NULL;
