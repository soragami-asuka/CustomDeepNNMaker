...
namespace NeuralNetwork {
	...
	/** 初期化. 各ニューロンの値をランダムに初期化
		...
		@return	成功した場合0 */
	ErrorCode BatchNormalization_LayerData_GPU::Initialize(const SettingData::Standard::IData& i_data)
	{
		ErrorCode err;
	...
	/** 初期化. バッファからデータを読み込む
		...
		@return	成功した場合0 */
	ErrorCode BatchNormalization_LayerData_GPU::InitializeFromBuffer(const BYTE* i_lpBuffer, U64 i_bufferSize, S64& o_useBufferSize )
	{
	...
	S64 BatchNormalization_LayerData_GPU::WriteToBuffer(BYTE* o_lpBuffer)const
	{
	...
	ILayerBase* BatchNormalization_LayerData_GPU::CreateLayer(const Gravisbell::GUID& guid, const IODataStruct i_lpInputDataStruct[], U32 i_inputLayerCount, Gravisbell::Common::ITemporaryMemoryManager& i_temporaryMemoryManager)
	{
		if(this->CheckCanUseInputDataStruct(i_lpInputDataStruct, i_inputLayerCount) == false)
			return NULL;

		return new CNNSingle2SingleLayerBase_GPU<BatchNormalization_GPU, BatchNormalization_LayerData_GPU>(guid, *this, i_lpInputDataStruct[0], i_temporaryMemoryManager);
	}
