...
namespace NeuralNetwork {
	...
	S32 BatchNormalization_GPU::WriteToBuffer(BYTE* o_lpBuffer)const
	{
	...
	/** 学習誤差を計算する.
		...
		直前の計算結果を使用する */
	ErrorCode BatchNormalization_GPU::CalculateLearnError(CONST_BATCH_BUFFER_POINTER i_lpDOutputBufferPrev)
	{
		cudnnStatus_t err_cudnn;
	...
	ErrorCode BatchNormalization_GPU::ReflectionLearnError(void)
	{
		return ErrorCode::ERROR_CODE_NONE;
	}
