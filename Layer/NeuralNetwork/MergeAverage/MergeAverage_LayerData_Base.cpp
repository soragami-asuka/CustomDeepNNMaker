...
namespace NeuralNetwork {
	...
	/** コンストラクタ */
	MergeAverage_LayerData_Base::MergeAverage_LayerData_Base(const Gravisbell::GUID& guid)
		:	guid	(guid)
	...
	/** デストラクタ */
	MergeAverage_LayerData_Base::~MergeAverage_LayerData_Base()
	{
	...
	//===========================
	/** レイヤー固有のGUIDを取得する */
	Gravisbell::GUID MergeAverage_LayerData_Base::GetGUID(void)const
	{
		return this->guid;
	}
	...
	/** レイヤー種別識別コードを取得する.
		...
		@return 成功した場合0 */
	Gravisbell::GUID MergeAverage_LayerData_Base::GetLayerCode(void)const
	{
	...
	/** 初期化. 各ニューロンの値をランダムに初期化
		@return	成功した場合0 */
	ErrorCode MergeAverage_LayerData_Base::Initialize(void)
	{
		// 出力データ構造を決定する
	...
	/** 初期化. 各ニューロンの値をランダムに初期化
		...
		@return	成功した場合0 */
	ErrorCode MergeAverage_LayerData_Base::Initialize(const SettingData::Standard::IData& i_data)
	{
		ErrorCode err;
	...
	/** 初期化. バッファからデータを読み込む
		...
		@return	成功した場合0 */
	ErrorCode MergeAverage_LayerData_Base::InitializeFromBuffer(const BYTE* i_lpBuffer, U64 i_bufferSize, S64& o_useBufferSize)
	{
	...
	//===========================
	/** 設定情報を設定 */
	ErrorCode MergeAverage_LayerData_Base::SetLayerConfig(const SettingData::Standard::IData& config)
	{
	...
	/** レイヤーの設定情報を取得する */
	const SettingData::Standard::IData* MergeAverage_LayerData_Base::GetLayerStructure()const
	{
	...
	U64 MergeAverage_LayerData_Base::GetUseBufferByteCount()const
	{
		U64 bufferSize = 0;
	...
	S64 MergeAverage_LayerData_Base::WriteToBuffer(BYTE* o_lpBuffer)const
	{
		if(this->pLayerStructure == NULL)
	...
	bool MergeAverage_LayerData_Base::CheckCanUseInputDataStruct(const IODataStruct i_lpInputDataStruct[], U32 i_inputLayerCount)
	{
	...
	IODataStruct MergeAverage_LayerData_Base::GetOutputDataStruct(const IODataStruct i_lpInputDataStruct[], U32 i_inputLayerCount)
	{
		...
		{
		case MergeAverage::LayerStructure::MergeType_max:
			...
			break;
		case MergeAverage::LayerStructure::MergeType_min:
			for(U32 inputNum=0; inputNum<i_inputLayerCount; inputNum++)
			{
				outputDataStruct.ch = min(outputDataStruct.ch, i_lpInputDataStruct[inputNum].ch);
			}
			break;
		case MergeAverage::LayerStructure::MergeType_layer0:
			outputDataStruct.ch = i_lpInputDataStruct[0].ch;
			break;
		}
	...
	/** 複数出力が可能かを確認する */
	bool MergeAverage_LayerData_Base::CheckCanHaveMultOutputLayer(void)
	{
		return false;
	}
