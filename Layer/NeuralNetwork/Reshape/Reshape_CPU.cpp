...
namespace NeuralNetwork {
	...
	/** コンストラクタ */
	Reshape_CPU::Reshape_CPU(Gravisbell::GUID guid, Reshape_LayerData_CPU& i_layerData, const IODataStruct& i_inputDataStruct)
		...
		,	m_lpDInputBuffer			(NULL)	/**< 演算時の出力誤差データ */
	{
	}
	/** デストラクタ */
	...
	/** 演算処理を実行する.
		...
		@return 成功した場合0が返る */
	ErrorCode Reshape_CPU::Calculate(CONST_BATCH_BUFFER_POINTER i_lpInputBuffer)
	{
	...
	CONST_BATCH_BUFFER_POINTER Reshape_CPU::GetOutputBuffer()const
	{
	...
	/** 出力データバッファを取得する.
		...
		@return 成功した場合0 */
	ErrorCode Reshape_CPU::GetOutputBuffer(BATCH_BUFFER_POINTER o_lpOutputBuffer)const
	{
	...
	/** 入力誤差計算をを実行する.学習せずに入力誤差を取得したい場合に使用する.
		...
		直前の計算結果を使用する */
	ErrorCode Reshape_CPU::CalculateDInput(BATCH_BUFFER_POINTER o_lppDInputBuffer, CONST_BATCH_BUFFER_POINTER i_lpDOutputBuffer)
	{
		// 入力誤差計算
		this->m_lpDInputBuffer = o_lppDInputBuffer;
		if(o_lppDInputBuffer)
		{
			// 出力誤差バッファを入力誤差バッファにコピー
			memcpy(o_lppDInputBuffer, i_lpDOutputBuffer, sizeof(F32)*this->GetInputBufferCount());
		}
	...
	/** 学習処理を実行する.
		...
		直前の計算結果を使用する */
	ErrorCode Reshape_CPU::Training(BATCH_BUFFER_POINTER o_lppDInputBuffer, CONST_BATCH_BUFFER_POINTER i_lpDOutputBuffer)
	{
	...
	CONST_BATCH_BUFFER_POINTER Reshape_CPU::GetDInputBuffer()const
	{
		return this->m_lpDInputBuffer;
	}
	/** 学習差分を取得する.
		@param lpDInputBuffer	学習差分を格納する配列.[GetBatchSize()の戻り値][GetInputBufferCount()の戻り値]の配列が必要 */
	ErrorCode Reshape_CPU::GetDInputBuffer(BATCH_BUFFER_POINTER o_lpDInputBuffer)const
	{
