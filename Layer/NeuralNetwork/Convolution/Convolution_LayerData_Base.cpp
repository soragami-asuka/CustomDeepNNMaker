...
namespace NeuralNetwork {
	...
	/** 初期化. 各ニューロンの値をランダムに初期化
		...
		@return	成功した場合0 */
	ErrorCode Convolution_LayerData_Base::Initialize(const SettingData::Standard::IData& i_data)
	{
		ErrorCode err;
	...
	/** 初期化. バッファからデータを読み込む
		...
		@return	成功した場合0 */
	ErrorCode Convolution_LayerData_Base::InitializeFromBuffer(const BYTE* i_lpBuffer, U64 i_bufferSize, S64& o_useBufferSize)
	{
	...
	U64 Convolution_LayerData_Base::GetUseBufferByteCount()const
	{
		U64 bufferSize = 0;
	...
	S64 Convolution_LayerData_Base::WriteToBuffer(BYTE* o_lpBuffer)const
	{
		if(this->pLayerStructure == NULL)
	...
	//===========================
	/** オプティマイザーを変更する */
	ErrorCode Convolution_LayerData_Base::ChangeOptimizer(const wchar_t i_optimizerID[])
	{
