...
namespace NeuralNetwork {
	...
	S32 Pooling_GPU::WriteToBuffer(BYTE* o_lpBuffer)const
	{
	...
	/** 学習誤差を計算する.
		...
		直前の計算結果を使用する */
	ErrorCode Pooling_GPU::CalculateLearnError(CONST_BATCH_BUFFER_POINTER i_lppDOutputBufferPrev)
	{
		// 出力誤差バッファのアドレスを配列に格納
	...
	ErrorCode Pooling_GPU::ReflectionLearnError(void)
	{
		return ErrorCode::ERROR_CODE_NONE;
	}
