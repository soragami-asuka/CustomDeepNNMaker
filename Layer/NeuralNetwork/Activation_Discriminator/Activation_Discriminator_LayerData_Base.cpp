...
//		,	layerStructure	()		/**< レイヤー構造 */
	...
	/** 初期化. バッファからデータを読み込む
		...
		@return	成功した場合0 */
	ErrorCode Activation_Discriminator_LayerData_Base::InitializeFromBuffer(const BYTE* i_lpBuffer, U64 i_bufferSize, S64& o_useBufferSize)
	{
...
//		this->pLayerStructure->WriteToStruct((BYTE*)&this->layerStructure);
	...
	U64 Activation_Discriminator_LayerData_Base::GetUseBufferByteCount()const
	{
		U64 bufferSize = 0;
	...
	S64 Activation_Discriminator_LayerData_Base::WriteToBuffer(BYTE* o_lpBuffer)const
	{
		if(this->pLayerStructure == NULL)
