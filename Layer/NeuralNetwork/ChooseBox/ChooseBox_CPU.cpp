...
#include"ChooseBox_CPU.h"
#include"ChooseBox_LayerData_CPU.h"
...
namespace NeuralNetwork {
	...
	/** コンストラクタ */
	ChooseBox_CPU::ChooseBox_CPU(Gravisbell::GUID guid, ChooseBox_LayerData_CPU& i_layerData, const IODataStruct& i_inputDataStruct, Gravisbell::Common::ITemporaryMemoryManager& i_temporaryMemoryManager)
	...
	//================================
	/** レイヤー種別の取得 */
	U32 ChooseBox_CPU::GetLayerKind()const
	{
	...
	/** 初期化. 各ニューロンの値をランダムに初期化
		@return	成功した場合0 */
	ErrorCode ChooseBox_CPU::Initialize(void)
	{
	...
	//===========================
	/** レイヤーデータを取得する */
	ChooseBox_LayerData_Base& ChooseBox_CPU::GetLayerData()
	{
		return this->layerData;
	}
	const ChooseBox_LayerData_Base& ChooseBox_CPU::GetLayerData()const
	{
		return this->layerData;
	}
	...
	ErrorCode ChooseBox_CPU::PreProcessLearn()
	{
	...
	ErrorCode ChooseBox_CPU::PreProcessCalculate()
	{
		// 入力バッファ数を確認
	...
	/** ループの初期化処理.データセットの実行開始前に実行する
		失敗した場合はCalculate以降の処理は実行不可. */
	ErrorCode ChooseBox_CPU::PreProcessLoop()
	{
	...
	/** 演算処理を実行する.
		...
		@return 成功した場合0が返る */
	ErrorCode ChooseBox_CPU::Calculate_device(CONST_BATCH_BUFFER_POINTER i_lppInputBuffer, BATCH_BUFFER_POINTER o_lppOutputBuffer)
	{
		...
		for(U32 batchNum=0; batchNum<this->GetBatchSize(); batchNum++)
		{
			...
			{
				for(S32 z=0; z<this->layerData.layerStructure.boxSize.z; z++)
				{
					U32 inputZ = this->layerData.layerStructure.startPosition.z + z;

					for(S32 y=0; y<this->layerData.layerStructure.boxSize.y; y++)
					{
						U32 inputY = this->layerData.layerStructure.startPosition.y + y;

						for(S32 x=0; x<this->layerData.layerStructure.boxSize.x; x++)
						{
	...
	/** 入力誤差計算をを実行する.学習せずに入力誤差を取得したい場合に使用する.
		...
		直前の計算結果を使用する */
	ErrorCode ChooseBox_CPU::CalculateDInput_device(CONST_BATCH_BUFFER_POINTER i_lppInputBuffer, BATCH_BUFFER_POINTER o_lppDInputBuffer, CONST_BATCH_BUFFER_POINTER i_lppOutputBuffer, CONST_BATCH_BUFFER_POINTER i_lppDOutputBuffer)
	{
		// 入力誤差計算
	...
	/** 学習処理を実行する.
		...
		直前の計算結果を使用する */
	ErrorCode ChooseBox_CPU::Training_device(CONST_BATCH_BUFFER_POINTER i_lppInputBuffer, BATCH_BUFFER_POINTER o_lppDInputBuffer, CONST_BATCH_BUFFER_POINTER i_lppOutputBuffer, CONST_BATCH_BUFFER_POINTER i_lppDOutputBuffer)
	{
