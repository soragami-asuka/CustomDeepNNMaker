...
namespace NeuralNetwork {
	...
	/** コンストラクタ */
	LayerConnectSingle2Mult::LayerConnectSingle2Mult(class FeedforwardNeuralNetwork_Base& neuralNetwork, ILayerBase* pLayer, bool onFixFlag)
	...
	//====================================
	/** 実行時設定を取得する. */
	const SettingData::Standard::IData* LayerConnectSingle2Mult::GetRuntimeParameter()const
	{
	...
	/** 実行時設定を設定する.
		...
		@param	i_param		設定する値. */
	ErrorCode LayerConnectSingle2Mult::SetRuntimeParameter(const wchar_t* i_dataID, S32 i_param)
	{
	...
	/** 実行時設定を設定する.
		...
		@param	i_dataID	設定する値のID.
		@param	i_param		設定する値. */
	ErrorCode LayerConnectSingle2Mult::SetRuntimeParameter(const wchar_t* i_dataID, F32 i_param)
	{
		return this->pLayer->SetRuntimeParameter(i_dataID, i_param);
	}
	/** 実行時設定を設定する.
		...
		@param	i_dataID	設定する値のID.
		@param	i_param		設定する値. */
	ErrorCode LayerConnectSingle2Mult::SetRuntimeParameter(const wchar_t* i_dataID, bool i_param)
	{
		return this->pLayer->SetRuntimeParameter(i_dataID, i_param);
	}
	/** 実行時設定を設定する.
		...
		@param	i_dataID	設定する値のID.
		@param	i_param		設定する値. */
	ErrorCode LayerConnectSingle2Mult::SetRuntimeParameter(const wchar_t* i_dataID, const wchar_t* i_param)
	{
		return this->pLayer->SetRuntimeParameter(i_dataID, i_param);
	}
	...
	/** 入力誤差バッファを位置指定で取得する */
	CONST_BATCH_BUFFER_POINTER LayerConnectSingle2Mult::GetDInputBufferByNum_d(S32 num)const
	{
	...
	bool LayerConnectSingle2Mult::IsFixLayer(void)const
	{
	...
	/** 入力誤差の計算が必要なフラグ.
		必要な場合trueが返る. */
	bool LayerConnectSingle2Mult::IsNecessaryCalculateDInput(void)const
	{
		if(this->lppInputFromLayer.empty())
			return false;
	...
	bool LayerConnectSingle2Mult::IsNecessaryBackPropagation(void)const
	{
	...
	ErrorCode LayerConnectSingle2Mult::SetOutputBufferID(S32 i_outputBufferID)
	{
	...
	/** 処理ループの初期化処理.
		失敗した場合はCalculate以降の処理は実行不可. */
	ErrorCode LayerConnectSingle2Mult::PreProcessLoop()
	{
	...
	ErrorCode LayerConnectSingle2Mult::CalculateDInput(void)
	{
		...
		for(U32 outputLayerNum=0; outputLayerNum<this->lppOutputToLayer.size(); outputLayerNum++)
		{
			this->lpDOutputBuffer[outputLayerNum] = this->lppOutputToLayer[outputLayerNum].pLayer->GetDInputBufferByNum_d(this->lppOutputToLayer[outputLayerNum].position);
		}
