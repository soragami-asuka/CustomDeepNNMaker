...
namespace NeuralNetwork {
	...
	/** コンストラクタ */
	Reshape_SquaresZeroSideLeftTop_CPU::Reshape_SquaresZeroSideLeftTop_CPU(Gravisbell::GUID guid, Reshape_SquaresZeroSideLeftTop_LayerData_CPU& i_layerData, const IODataStruct& i_inputDataStruct)
	...
	/** デストラクタ */
	Reshape_SquaresZeroSideLeftTop_CPU::~Reshape_SquaresZeroSideLeftTop_CPU()
	{
	}
	...
	//================================
	/** レイヤー種別の取得 */
	U32 Reshape_SquaresZeroSideLeftTop_CPU::GetLayerKind()const
	{
	...
	/** 初期化. 各ニューロンの値をランダムに初期化
		@return	成功した場合0 */
	ErrorCode Reshape_SquaresZeroSideLeftTop_CPU::Initialize(void)
	{
	...
	//===========================
	/** レイヤーデータを取得する */
	Reshape_SquaresZeroSideLeftTop_LayerData_Base& Reshape_SquaresZeroSideLeftTop_CPU::GetLayerData()
	{
		return this->layerData;
	}
	const Reshape_SquaresZeroSideLeftTop_LayerData_Base& Reshape_SquaresZeroSideLeftTop_CPU::GetLayerData()const
	{
		return this->layerData;
	}
	...
	ErrorCode Reshape_SquaresZeroSideLeftTop_CPU::PreProcessLearn()
	{
	...
	ErrorCode Reshape_SquaresZeroSideLeftTop_CPU::PreProcessCalculate()
	{
		// 入力バッファ数を確認
	...
	/** ループの初期化処理.データセットの実行開始前に実行する
		失敗した場合はCalculate以降の処理は実行不可. */
	ErrorCode Reshape_SquaresZeroSideLeftTop_CPU::PreProcessLoop()
	{
	...
	/** 演算処理を実行する.
		...
		@return 成功した場合0が返る */
	ErrorCode Reshape_SquaresZeroSideLeftTop_CPU::Calculate(CONST_BATCH_BUFFER_POINTER i_lpInputBuffer)
	{
		// 入力バッファのアドレスを配列に格納
	...
	CONST_BATCH_BUFFER_POINTER Reshape_SquaresZeroSideLeftTop_CPU::GetOutputBuffer()const
	{
	...
	/** 出力データバッファを取得する.
		...
		@return 成功した場合0 */
	ErrorCode Reshape_SquaresZeroSideLeftTop_CPU::GetOutputBuffer(BATCH_BUFFER_POINTER o_lpOutputBuffer)const
	{
	...
	/** 入力誤差計算をを実行する.学習せずに入力誤差を取得したい場合に使用する.
		...
		直前の計算結果を使用する */
	ErrorCode Reshape_SquaresZeroSideLeftTop_CPU::CalculateDInput(BATCH_BUFFER_POINTER o_lppDInputBuffer, CONST_BATCH_BUFFER_POINTER i_lpDOutputBuffer)
	{
		// 入力誤差計算
	...
	/** 学習処理を実行する.
		...
		直前の計算結果を使用する */
	ErrorCode Reshape_SquaresZeroSideLeftTop_CPU::Training(BATCH_BUFFER_POINTER o_lppDInputBuffer, CONST_BATCH_BUFFER_POINTER i_lpDOutputBuffer)
	{
	...
	CONST_BATCH_BUFFER_POINTER Reshape_SquaresZeroSideLeftTop_CPU::GetDInputBuffer()const
	{
		return this->m_lpInputBuffer;
	}
	/** 学習差分を取得する.
		@param lpDInputBuffer	学習差分を格納する配列.[GetBatchSize()の戻り値][GetInputBufferCount()の戻り値]の配列が必要 */
	ErrorCode Reshape_SquaresZeroSideLeftTop_CPU::GetDInputBuffer(BATCH_BUFFER_POINTER o_lpDInputBuffer)const
	{
