...
#define CALC_INPUT_MAX	(1024)
	...
	__global__ void device_ReLimitBackPropagationBox(
		U32 chCount,
	...
	/** コンストラクタ */
	LimitBackPropagationBox_GPU::LimitBackPropagationBox_GPU(Gravisbell::GUID guid, LimitBackPropagationBox_LayerData_GPU& i_layerData, const IODataStruct& i_inputDataStruct, Gravisbell::Common::ITemporaryMemoryManager& i_temporaryMemoryManager)
	...
	/** デストラクタ */
	LimitBackPropagationBox_GPU::~LimitBackPropagationBox_GPU()
	{
	...
	//================================
	/** レイヤー種別の取得 */
	U32 LimitBackPropagationBox_GPU::GetLayerKind()const
	{
	...
	/** 初期化. 各ニューロンの値をランダムに初期化
		@return	成功した場合0 */
	ErrorCode LimitBackPropagationBox_GPU::Initialize(void)
	{
	...
	//===========================
	/** レイヤーデータを取得する */
	LimitBackPropagationBox_LayerData_Base& LimitBackPropagationBox_GPU::GetLayerData()
	{
		return this->layerData;
	}
	const LimitBackPropagationBox_LayerData_Base& LimitBackPropagationBox_GPU::GetLayerData()const
	{
		return this->layerData;
	}
	...
	ErrorCode LimitBackPropagationBox_GPU::PreProcessLearn()
	{
	...
	ErrorCode LimitBackPropagationBox_GPU::PreProcessCalculate()
	{
		// 入力バッファ数を確認
	...
	/** ループの初期化処理.データセットの実行開始前に実行する
		失敗した場合はCalculate以降の処理は実行不可. */
	ErrorCode LimitBackPropagationBox_GPU::PreProcessLoop()
	{
		return ErrorCode::ERROR_CODE_NONE;
	}
	...
	/** 演算処理を実行する.
		...
		@return 成功した場合0が返る */
	ErrorCode LimitBackPropagationBox_GPU::Calculate_device(CONST_BATCH_BUFFER_POINTER i_lppInputBuffer, BATCH_BUFFER_POINTER o_lppOutputBuffer)
	{
		// 入力バッファを出力バッファにコピー
	...
	/** 入力誤差計算をを実行する.学習せずに入力誤差を取得したい場合に使用する.
		...
		直前の計算結果を使用する */
	ErrorCode LimitBackPropagationBox_GPU::CalculateDInput_device(CONST_BATCH_BUFFER_POINTER i_lppInputBuffer, BATCH_BUFFER_POINTER o_lppDInputBuffer, CONST_BATCH_BUFFER_POINTER i_lppOutputBuffer, CONST_BATCH_BUFFER_POINTER i_lppDOutputBuffer)
	{
		// 入力誤差計算
		if(o_lppDInputBuffer)
		{
			...
			device_ReLimitBackPropagationBox<<<grid, block>>>(
	...
	/** 学習処理を実行する.
		...
		直前の計算結果を使用する */
	ErrorCode LimitBackPropagationBox_GPU::Training_device(CONST_BATCH_BUFFER_POINTER i_lppInputBuffer, BATCH_BUFFER_POINTER o_lppDInputBuffer, CONST_BATCH_BUFFER_POINTER i_lppOutputBuffer, CONST_BATCH_BUFFER_POINTER i_lppDOutputBuffer)
	{
