...
//		,	layerStructure	()		/**< レイヤー構造 */
	...
	/** 初期化. 各ニューロンの値をランダムに初期化
		...
		@return	成功した場合0 */
	ErrorCode Residual_LayerData_Base::Initialize(const SettingData::Standard::IData& i_data)
	{
		ErrorCode err;
	...
	/** 初期化. バッファからデータを読み込む
		...
		@return	成功した場合0 */
	ErrorCode Residual_LayerData_Base::InitializeFromBuffer(const BYTE* i_lpBuffer, U64 i_bufferSize, S64& o_useBufferSize)
	{
...
//		this->pLayerStructure->WriteToStruct((BYTE*)&this->layerStructure);
	...
	U64 Residual_LayerData_Base::GetUseBufferByteCount()const
	{
		U64 bufferSize = 0;
	...
	S64 Residual_LayerData_Base::WriteToBuffer(BYTE* o_lpBuffer)const
	{
		if(this->pLayerStructure == NULL)
	...
	bool Residual_LayerData_Base::CheckCanUseInputDataStruct(const IODataStruct i_lpInputDataStruct[], U32 i_inputLayerCount)
	{
	...
	IODataStruct Residual_LayerData_Base::GetOutputDataStruct(const IODataStruct i_lpInputDataStruct[], U32 i_inputLayerCount)
	{
	...
	/** 複数出力が可能かを確認する */
	bool Residual_LayerData_Base::CheckCanHaveMultOutputLayer(void)
	{
		return false;
	}
