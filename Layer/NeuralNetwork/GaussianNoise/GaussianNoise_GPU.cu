...
namespace NeuralNetwork {
	...
	/** コンストラクタ */
	GaussianNoise_GPU::GaussianNoise_GPU(Gravisbell::GUID guid, GaussianNoise_LayerData_GPU& i_layerData, const IODataStruct& i_inputDataStruct, Gravisbell::Common::ITemporaryMemoryManager& i_temporaryMemoryManager)
	...
	/** 演算処理を実行する.
		...
		@return 成功した場合0が返る */
	ErrorCode GaussianNoise_GPU::Calculate_device(CONST_BATCH_BUFFER_POINTER i_lppInputBuffer, BATCH_BUFFER_POINTER o_lppOutputBuffer)
	{
		// 入力バッファを出力バッファにコピー
		...
		RandomGenerator<<<grid, block>>>(0, average, variance, o_lppOutputBuffer, this->outputBufferCount*this->GetBatchSize());

#ifdef _DEBUG
		std::vector<F32> lpTmpInputBuffer(this->inputBufferCount*this->GetBatchSize());
		cudaMemcpy(&lpTmpInputBuffer[0], i_lppInputBuffer, sizeof(F32)*lpTmpInputBuffer.size(), cudaMemcpyDeviceToHost);
	...
	/** 入力誤差計算をを実行する.学習せずに入力誤差を取得したい場合に使用する.
		...
		直前の計算結果を使用する */
	ErrorCode GaussianNoise_GPU::CalculateDInput_device(CONST_BATCH_BUFFER_POINTER i_lppInputBuffer, BATCH_BUFFER_POINTER o_lppDInputBuffer, CONST_BATCH_BUFFER_POINTER i_lppOutputBuffer, CONST_BATCH_BUFFER_POINTER i_lppDOutputBuffer)
	{
		// 入力誤差計算
	...
	/** 学習処理を実行する.
		...
		@param	i_lppDOutputBuffer	出力誤差差分=次レイヤーの入力誤差差分.	[GetBatchSize()の戻り値][GetOutputBufferCount()の戻り値]の要素数が必要.
		直前の計算結果を使用する */
	ErrorCode GaussianNoise_GPU::Training_device(CONST_BATCH_BUFFER_POINTER i_lppInputBuffer, BATCH_BUFFER_POINTER o_lppDInputBuffer, CONST_BATCH_BUFFER_POINTER i_lppOutputBuffer, CONST_BATCH_BUFFER_POINTER i_lppDOutputBuffer)
	{
