...
namespace NeuralNetwork {
	...
	/** コンストラクタ */
	ChooseChannel_GPU::ChooseChannel_GPU(Gravisbell::GUID guid, ChooseChannel_LayerData_GPU& i_layerData, const IODataStruct& i_inputDataStruct, Gravisbell::Common::ITemporaryMemoryManager& i_temporaryMemoryManager)
	...
	/** 演算処理を実行する.
		...
		@return 成功した場合0が返る */
	ErrorCode ChooseChannel_GPU::Calculate_device(CONST_BATCH_BUFFER_POINTER i_lppInputBuffer, BATCH_BUFFER_POINTER o_lppOutputBuffer)
	{
		...
		{
			cudaMemcpy(
				this->lppBatchOutputBuffer[batchNum],
				&this->lppBatchInputBuffer[batchNum][this->layerData.layerStructure.startChannelNo*this->channelSize],
	...
	/** 入力誤差計算をを実行する.学習せずに入力誤差を取得したい場合に使用する.
		...
		直前の計算結果を使用する */
	ErrorCode ChooseChannel_GPU::CalculateDInput_device(CONST_BATCH_BUFFER_POINTER i_lppInputBuffer, BATCH_BUFFER_POINTER o_lppDInputBuffer, CONST_BATCH_BUFFER_POINTER i_lppOutputBuffer, CONST_BATCH_BUFFER_POINTER i_lppDOutputBuffer)
	{
		// 入力誤差計算
		if(o_lppDInputBuffer)
		{
			...
			{
				cudaMemcpy(
					&this->lppBatchDInputBuffer[batchNum][this->layerData.layerStructure.startChannelNo*this->channelSize],
	...
	/** 学習処理を実行する.
		...
		直前の計算結果を使用する */
	ErrorCode ChooseChannel_GPU::Training_device(CONST_BATCH_BUFFER_POINTER i_lppInputBuffer, BATCH_BUFFER_POINTER o_lppDInputBuffer, CONST_BATCH_BUFFER_POINTER i_lppOutputBuffer, CONST_BATCH_BUFFER_POINTER i_lppDOutputBuffer)
	{
