...
namespace NeuralNetwork {
	...
	/** コンストラクタ */
	BatchNormalizationAll_LayerData_Base::BatchNormalizationAll_LayerData_Base(const Gravisbell::GUID& guid)
		:	guid	(guid)
	...
	/** デストラクタ */
	BatchNormalizationAll_LayerData_Base::~BatchNormalizationAll_LayerData_Base()
	{
	...
	//===========================
	/** レイヤー固有のGUIDを取得する */
	Gravisbell::GUID BatchNormalizationAll_LayerData_Base::GetGUID(void)const
	{
		return this->guid;
	}
	...
	/** レイヤー種別識別コードを取得する.
		...
		@return 成功した場合0 */
	Gravisbell::GUID BatchNormalizationAll_LayerData_Base::GetLayerCode(void)const
	{
	...
	//===========================
	/** 設定情報を設定 */
	ErrorCode BatchNormalizationAll_LayerData_Base::SetLayerConfig(const SettingData::Standard::IData& config)
	{
	...
	/** レイヤーの設定情報を取得する */
	const SettingData::Standard::IData* BatchNormalizationAll_LayerData_Base::GetLayerStructure()const
	{
	...
	U64 BatchNormalizationAll_LayerData_Base::GetUseBufferByteCount()const
	{
		U64 bufferSize = 0;
		...
		// 各データ数
		bufferSize += sizeof(F32) * 1;	// 平均
		bufferSize += sizeof(F32) * 1;	// 分散
	...
	bool BatchNormalizationAll_LayerData_Base::CheckCanUseInputDataStruct(const IODataStruct i_lpInputDataStruct[], U32 i_inputLayerCount)
	{
	...
	IODataStruct BatchNormalizationAll_LayerData_Base::GetOutputDataStruct(const IODataStruct i_lpInputDataStruct[], U32 i_inputLayerCount)
	{
	...
	/** 複数出力が可能かを確認する */
	bool BatchNormalizationAll_LayerData_Base::CheckCanHaveMultOutputLayer(void)
	{
		return false;
	}
	...
	ErrorCode BatchNormalizationAll_LayerData_Base::SetOptimizerHyperParameter(const wchar_t i_parameterID[], F32 i_value)
	{
		if(this->m_pOptimizer_bias)
	...
	ErrorCode BatchNormalizationAll_LayerData_Base::SetOptimizerHyperParameter(const wchar_t i_parameterID[], S32 i_value)
	{
		if(this->m_pOptimizer_bias)
			this->m_pOptimizer_bias->SetHyperParameter(i_parameterID, i_value);
		if(this->m_pOptimizer_scale)
	...
	ErrorCode BatchNormalizationAll_LayerData_Base::SetOptimizerHyperParameter(const wchar_t i_parameterID[], const wchar_t i_value[])
	{
		if(this->m_pOptimizer_bias)
			this->m_pOptimizer_bias->SetHyperParameter(i_parameterID, i_value);
		if(this->m_pOptimizer_scale)
