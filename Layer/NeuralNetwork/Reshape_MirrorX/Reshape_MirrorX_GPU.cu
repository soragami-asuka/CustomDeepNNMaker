...
namespace NeuralNetwork {
	...
	/** コンストラクタ */
	Reshape_MirrorX_GPU::Reshape_MirrorX_GPU(Gravisbell::GUID guid, Reshape_MirrorX_LayerData_GPU& i_layerData, const IODataStruct& i_inputDataStruct)
	...
	/** 演算処理を実行する.
		...
		@return 成功した場合0が返る */
	ErrorCode Reshape_MirrorX_GPU::Calculate(CONST_BATCH_BUFFER_POINTER i_lpInputBuffer)
	{
	...
	CONST_BATCH_BUFFER_POINTER Reshape_MirrorX_GPU::GetOutputBuffer()const
	{
	...
	/** 出力データバッファを取得する.
		...
		@return 成功した場合0 */
	ErrorCode Reshape_MirrorX_GPU::GetOutputBuffer(BATCH_BUFFER_POINTER o_lpOutputBuffer)const
	{
	...
	/** 入力誤差計算をを実行する.学習せずに入力誤差を取得したい場合に使用する.
		...
		直前の計算結果を使用する */
	ErrorCode Reshape_MirrorX_GPU::CalculateDInput(BATCH_BUFFER_POINTER o_lppDInputBuffer, CONST_BATCH_BUFFER_POINTER i_lpDOutputBuffer)
	{
		// 入力誤差計算
	...
	/** 学習処理を実行する.
		...
		直前の計算結果を使用する */
	ErrorCode Reshape_MirrorX_GPU::Training(BATCH_BUFFER_POINTER o_lppDInputBuffer, CONST_BATCH_BUFFER_POINTER i_lpDOutputBuffer)
	{
	...
	CONST_BATCH_BUFFER_POINTER Reshape_MirrorX_GPU::GetDInputBuffer()const
	{
		return this->m_lpDOutputBuffer;
	}
	/** 学習差分を取得する.
		@param lpDInputBuffer	学習差分を格納する配列.[GetBatchSize()の戻り値][GetInputBufferCount()の戻り値]の配列が必要 */
	ErrorCode Reshape_MirrorX_GPU::GetDInputBuffer(BATCH_BUFFER_POINTER o_lpDInputBuffer)const
	{
