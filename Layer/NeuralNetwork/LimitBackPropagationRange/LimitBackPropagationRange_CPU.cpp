...
namespace NeuralNetwork {
	...
	/** コンストラクタ */
	LimitBackPropagationRange_CPU::LimitBackPropagationRange_CPU(Gravisbell::GUID guid, LimitBackPropagationRange_LayerData_CPU& i_layerData, const IODataStruct& i_inputDataStruct, Gravisbell::Common::ITemporaryMemoryManager& i_temporaryMemoryManager)
	...
	/** デストラクタ */
	LimitBackPropagationRange_CPU::~LimitBackPropagationRange_CPU()
	{
	}
	...
	//================================
	/** レイヤー種別の取得 */
	U32 LimitBackPropagationRange_CPU::GetLayerKind()const
	{
	...
	/** 初期化. 各ニューロンの値をランダムに初期化
		@return	成功した場合0 */
	ErrorCode LimitBackPropagationRange_CPU::Initialize(void)
	{
	...
	//===========================
	/** レイヤーデータを取得する */
	LimitBackPropagationRange_LayerData_Base& LimitBackPropagationRange_CPU::GetLayerData()
	{
		return this->layerData;
	}
	const LimitBackPropagationRange_LayerData_Base& LimitBackPropagationRange_CPU::GetLayerData()const
	{
		return this->layerData;
	}
	...
	ErrorCode LimitBackPropagationRange_CPU::PreProcessLearn()
	{
	...
	ErrorCode LimitBackPropagationRange_CPU::PreProcessCalculate()
	{
		// 入力バッファ数を確認
	...
	/** ループの初期化処理.データセットの実行開始前に実行する
		失敗した場合はCalculate以降の処理は実行不可. */
	ErrorCode LimitBackPropagationRange_CPU::PreProcessLoop()
	{
	...
	/** 演算処理を実行する.
		...
		@return 成功した場合0が返る */
	ErrorCode LimitBackPropagationRange_CPU::Calculate_device(CONST_BATCH_BUFFER_POINTER i_lppInputBuffer, BATCH_BUFFER_POINTER o_lppOutputBuffer)
	{
	...
	/** 入力誤差計算をを実行する.学習せずに入力誤差を取得したい場合に使用する.
		...
		直前の計算結果を使用する */
	ErrorCode LimitBackPropagationRange_CPU::CalculateDInput_device(CONST_BATCH_BUFFER_POINTER i_lppInputBuffer, BATCH_BUFFER_POINTER o_lppDInputBuffer, CONST_BATCH_BUFFER_POINTER i_lppOutputBuffer, CONST_BATCH_BUFFER_POINTER i_lppDOutputBuffer)
	{
		// 入力誤差計算
	...
	/** 学習処理を実行する.
		...
		直前の計算結果を使用する */
	ErrorCode LimitBackPropagationRange_CPU::Training_device(CONST_BATCH_BUFFER_POINTER i_lppInputBuffer, BATCH_BUFFER_POINTER o_lppDInputBuffer, CONST_BATCH_BUFFER_POINTER i_lppOutputBuffer, CONST_BATCH_BUFFER_POINTER i_lppDOutputBuffer)
	{
