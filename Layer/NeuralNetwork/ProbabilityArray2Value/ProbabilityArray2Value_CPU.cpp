...
#include"ProbabilityArray2Value_CPU.h"
#include"ProbabilityArray2Value_LayerData_CPU.h"
...
namespace NeuralNetwork {
	...
	/** コンストラクタ */
	ProbabilityArray2Value_CPU::ProbabilityArray2Value_CPU(Gravisbell::GUID guid, ProbabilityArray2Value_LayerData_CPU& i_layerData, const IODataStruct& i_inputDataStruct, Gravisbell::Common::ITemporaryMemoryManager& i_temporaryMemoryManager)
	...
	/** デストラクタ */
	ProbabilityArray2Value_CPU::~ProbabilityArray2Value_CPU()
	{
	}
	...
	//================================
	/** レイヤー種別の取得 */
	U32 ProbabilityArray2Value_CPU::GetLayerKind()const
	{
	...
	/** 初期化. 各ニューロンの値をランダムに初期化
		@return	成功した場合0 */
	ErrorCode ProbabilityArray2Value_CPU::Initialize(void)
	{
	...
	//===========================
	/** レイヤーデータを取得する */
	ProbabilityArray2Value_LayerData_Base& ProbabilityArray2Value_CPU::GetLayerData()
	{
		return this->layerData;
	}
	const ProbabilityArray2Value_LayerData_Base& ProbabilityArray2Value_CPU::GetLayerData()const
	{
		return this->layerData;
	}
	...
	ErrorCode ProbabilityArray2Value_CPU::PreProcessLearn()
	{
		...
		// 一時計算バッファを確保
		this->lpTmpBuffer.resize(this->GetInputDataStruct().ch);
	...
	ErrorCode ProbabilityArray2Value_CPU::PreProcessCalculate()
	{
		// 入力バッファ数を確認
	...
	/** ループの初期化処理.データセットの実行開始前に実行する
		失敗した場合はCalculate以降の処理は実行不可. */
	ErrorCode ProbabilityArray2Value_CPU::PreProcessLoop()
	{
	...
	/** 演算処理を実行する.
		...
		@return 成功した場合0が返る */
	ErrorCode ProbabilityArray2Value_CPU::Calculate_device(CONST_BATCH_BUFFER_POINTER i_lppInputBuffer, BATCH_BUFFER_POINTER o_lppOutputBuffer)
	{
	...
	/** 入力誤差計算をを実行する.学習せずに入力誤差を取得したい場合に使用する.
		...
		直前の計算結果を使用する */
	ErrorCode ProbabilityArray2Value_CPU::CalculateDInput_device(CONST_BATCH_BUFFER_POINTER i_lppInputBuffer, BATCH_BUFFER_POINTER o_lppDInputBuffer, CONST_BATCH_BUFFER_POINTER i_lppOutputBuffer, CONST_BATCH_BUFFER_POINTER i_lppDOutputBuffer)
	{
		// 入力誤差計算
					...
					for(U32 y=0; y<this->GetInputDataStruct().y; y++)
					{
						for(U32 x=0; x<this->GetInputDataStruct().x; x++)
						{
							...
							for(U32 ch=0; ch<this->GetInputDataStruct().ch; ch++)
							{
								...
								F32 value        = ((float)ch / this->GetInputDataStruct().ch)
												 * (this->layerData.layerStructure.outputMaxValue - this->layerData.layerStructure.outputMinValue)
												 + this->layerData.layerStructure.outputMinValue;

								// 正規分布を求める
								this->lpTmpBuffer[ch] = 1.0f / (2.0f * (F32)M_PI * this->layerData.layerStructure.variance) * expf(-powf(value - trueValue,2) / (2.0f * this->layerData.layerStructure.variance * this->layerData.layerStructure.variance));
								sumValue += this->lpTmpBuffer[ch];
							}
							...
							for(U32 ch=0; ch<this->GetInputDataStruct().ch; ch++)
							{
								...
								this->lppBatchDInputBuffer[batchNum][inputOffset] = this->lpTmpBuffer[ch] / sumValue;
							}
						}
					}
				}
	...
	/** 学習処理を実行する.
		...
		直前の計算結果を使用する */
	ErrorCode ProbabilityArray2Value_CPU::Training_device(CONST_BATCH_BUFFER_POINTER i_lppInputBuffer, BATCH_BUFFER_POINTER o_lppDInputBuffer, CONST_BATCH_BUFFER_POINTER i_lppOutputBuffer, CONST_BATCH_BUFFER_POINTER i_lppDOutputBuffer)
	{
