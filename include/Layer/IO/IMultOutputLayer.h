...
namespace IO {
	...
	public:
		...
		/** 出力データ構造を取得する.
			@return	出力データ構造 */
		virtual IODataStruct GetOutputDataStruct(U32 i_layerNum)const = 0;

		/** 出力バッファ数を取得する. byte数では無くデータの数なので注意 */
		virtual U32 GetOutputBufferCount(U32 i_layerNum)const = 0;

		/** 出力データバッファを取得する.
			配列の要素数は[GetBatchSize()の戻り値][GetOutputBufferCount()の戻り値]
			@return 出力データ配列の先頭ポインタ */
		virtual CONST_BATCH_BUFFER_POINTER GetOutputBuffer(U32 i_layerNum)const = 0;
		/** 出力データバッファを取得する.
