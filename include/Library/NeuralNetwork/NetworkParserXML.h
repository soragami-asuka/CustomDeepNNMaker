...
namespace Parser {
	...
	/** レイヤーデータをXMLファイルから作成する.
		...
		*/
	extern NetworkParserXML_API ILayerData* CreateLayerFromXML(const ILayerDLLManager& i_layerDLLManager, ILayerDataManager& io_layerDataManager, const wchar_t i_layerDirPath[], const wchar_t i_rootLayerFilePath[]);
	...
	/** レイヤーデータをXMLファイルに書き出す.
		...
		*/
	extern NetworkParserXML_API Gravisbell::ErrorCode SaveLayerToXML(ILayerData& i_NNLayer, const wchar_t i_layerDirPath[], const wchar_t i_rootLayerFilePath[] = L"");
