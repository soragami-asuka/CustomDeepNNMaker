...
namespace NeuralNetworkLayer {
	...
	public:
		...
		virtual Gravisbell::GUID AddConvolutionLayer(
			...
			const wchar_t i_szWeightData[] = L"Default", const wchar_t i_szInitializerID[] = L"glorot_uniform") = 0;
		...
		virtual Gravisbell::GUID AddFullyConnectLayer(const Gravisbell::GUID& i_inputLayerGUID, U32 i_neuronCount, const wchar_t i_szWeightData[] = L"Default", const wchar_t i_szInitializerID[] = L"glorot_uniform") = 0;
		...
		/** 後方伝搬範囲制限レイヤー. 出力レイヤーの特定XYZ区間以外の後方伝搬を停止する. 入力/出力データ構造でCH,x,y,zは同じサイズ.
			@param	startPosition	開始XYZ位置.
			@param	boxSize			抽出XYZ数. */
		virtual Gravisbell::GUID AddLimitBackPropagationBoxLayer(const Gravisbell::GUID& i_inputLayerGUID, Vector3D<S32> startPosition, Vector3D<S32> boxSize) = 0;
	...
	public:
		...
		virtual Gravisbell::GUID AddMergeAddLayer(LayerMergeType i_layerMergeType, Gravisbell::F32 i_scale, const Gravisbell::GUID lpInputLayerGUID[], U32 inputLayerCount) = 0;
		...
		{
			return AddMergeAddLayer(i_layerMergeType, 1.0f, lpInputLayerGUID, inputLayerCount);
		}

		/** 入力結合レイヤー. 入力されたレイヤーの値を合算する. 入力データ構造はX,Y,Zで同じサイズである必要がある. */
		Gravisbell::GUID AddMergeAddLayer(LayerMergeType i_layerMergeType, Gravisbell::F32 i_scale, const std::vector<Gravisbell::GUID>& lpInputLayerGUID)
		{
			return AddMergeAddLayer(i_layerMergeType, i_scale, &lpInputLayerGUID[0], (U32)lpInputLayerGUID.size());
		}
		Gravisbell::GUID AddMergeAddLayer(LayerMergeType i_layerMergeType, const std::vector<Gravisbell::GUID>& lpInputLayerGUID)
		{
			return AddMergeAddLayer(i_layerMergeType, 1.0f, &lpInputLayerGUID[0], (U32)lpInputLayerGUID.size());
		}

		/** 入力結合レイヤー. 入力されたレイヤーの値を合算する. 入力データ構造はX,Y,Zで同じサイズである必要がある. */
		template<typename... Rest>
		Gravisbell::GUID AddMergeAddLayer(LayerMergeType i_layerMergeType, Gravisbell::F32 i_scale, std::vector<Gravisbell::GUID>& lpInputLayerGUID, const Gravisbell::GUID& lastLayerGUID_first, const Rest&... lpLastLayerGUID_rest)
		{
			lpInputLayerGUID.push_back(lastLayerGUID_first);

			return AddMergeAddLayer(i_layerMergeType, i_scale, lpInputLayerGUID, lpLastLayerGUID_rest...);
		}
		template<typename... Rest>
		Gravisbell::GUID AddMergeAddLayer(LayerMergeType i_layerMergeType, std::vector<Gravisbell::GUID>& lpInputLayerGUID, const Gravisbell::GUID& lastLayerGUID_first, const Rest&... lpLastLayerGUID_rest)
		{
			...
			return AddMergeAddLayer(i_layerMergeType, 1.0f, lpInputLayerGUID, lpLastLayerGUID_rest...);
		}
	...
	public:
		...
		/** 入力結合レイヤー. 入力されたレイヤーの値を合算する. 入力データ構造はX,Y,Zで同じサイズである必要がある.
			...
			*/
		template<typename... Rest>
		Gravisbell::GUID AddMergeAddLayer(LayerMergeType i_layerMergeType, F32 i_scale, const Gravisbell::GUID& lastLayerGUID_first, const Rest&... lpLastLayerGUID_rest)
		{
			std::vector<Gravisbell::GUID> lpInputLayerGUID;
			lpInputLayerGUID.push_back(lastLayerGUID_first);
	...
	public:
		...
		virtual Gravisbell::GUID AddNeuralNetworkLayer_CA(const GUID& i_inputLayerGUID, Vector3D<S32> i_filterSize, U32 i_outputChannelCount, Vector3D<S32> i_stride, Vector3D<S32> i_paddingSize, const wchar_t i_activationType[], const wchar_t i_szWeightData[] = L"Default", const wchar_t i_szInitializerID[] = L"glorot_uniform") = 0;

		/** ニューラルネットワークにConvolution, BatchNormalization, Activationを行うレイヤーを追加する. */
		virtual Gravisbell::GUID AddNeuralNetworkLayer_CBA(const GUID& i_inputLayerGUID, Vector3D<S32> i_filterSize, U32 i_outputChannelCount, Vector3D<S32> i_stride, Vector3D<S32> i_paddingSize, const wchar_t i_activationType[], const wchar_t i_szWeightData[] = L"Default", const wchar_t i_szInitializerID[] = L"glorot_uniform") = 0;

		/** ニューラルネットワークにConvolution, BatchNormalization, Noise, Activationを行うレイヤーを追加する. */
		virtual Gravisbell::GUID AddNeuralNetworkLayer_CBNA(const GUID& i_inputLayerGUID, Vector3D<S32> i_filterSize, U32 i_outputChannelCount, Vector3D<S32> i_stride, Vector3D<S32> i_paddingSize, const wchar_t i_activationType[], Gravisbell::F32 i_noiseVariance, const wchar_t i_szWeightData[] = L"Default", const wchar_t i_szInitializerID[] = L"glorot_uniform") = 0;

		/** ニューラルネットワークにBatchNormalization, Activation, Convolutionを行うレイヤーを追加する. */
		virtual Gravisbell::GUID AddNeuralNetworkLayer_BAC(const GUID& i_inputLayerGUID, Vector3D<S32> i_filterSize, U32 i_outputChannelCount, Vector3D<S32> i_stride, Vector3D<S32> i_paddingSize, const wchar_t i_activationType[], const wchar_t i_szWeightData[] = L"Default", const wchar_t i_szInitializerID[] = L"glorot_uniform") = 0;

		/** ニューラルネットワークにBatchNormalization, Activation. Fully-Connectを行うレイヤーを追加する. */
		virtual Gravisbell::GUID AddNeuralNetworkLayer_BAF(const GUID& i_inputLayerGUID, U32 i_outputChannelCount, const wchar_t i_activationType[], const wchar_t i_szWeightData[] = L"Default", const wchar_t i_szInitializerID[] = L"glorot_uniform") = 0;

		/** ニューラルネットワークにFully-Connect, Activationを行うレイヤーを追加する. */
		virtual Gravisbell::GUID AddNeuralNetworkLayer_FA(const GUID& i_inputLayerGUID, U32 i_outputChannelCount, const wchar_t i_activationType[], const wchar_t i_szWeightData[] = L"Default", const wchar_t i_szInitializerID[] = L"glorot_uniform") = 0;

		/** ニューラルネットワークにFully-Connect, Activation, Dropoutを行うレイヤーを追加する. */
		virtual Gravisbell::GUID AddNeuralNetworkLayer_FAD(const GUID& i_inputLayerGUID, U32 i_outputChannelCount, const wchar_t i_activationType[], F32 i_dropOutRate, const wchar_t i_szWeightData[] = L"Default", const wchar_t i_szInitializerID[] = L"glorot_uniform") = 0;

		/** ニューラルネットワークにBatchNormalization, Noise, Activation, Convolutionを行うレイヤーを追加する. */
		virtual Gravisbell::GUID AddNeuralNetworkLayer_BNAC(const GUID& i_inputLayerGUID, Vector3D<S32> i_filterSize, U32 i_outputChannelCount, Vector3D<S32> i_stride, Vector3D<S32> i_paddingSize, const wchar_t i_activationType[], F32 i_noiseVariance, const wchar_t i_szWeightData[] = L"Default", const wchar_t i_szInitializerID[] = L"glorot_uniform") = 0;

		/** ニューラルネットワークにNoise, BatchNormalization, Activation, Convolutionを行うレイヤーを追加する. */
		virtual Gravisbell::GUID AddNeuralNetworkLayer_NBAC(const GUID& i_inputLayerGUID, Vector3D<S32> i_filterSize, U32 i_outputChannelCount, Vector3D<S32> i_stride, Vector3D<S32> i_paddingSize, const wchar_t i_activationType[], F32 i_noiseVariance, const wchar_t i_szWeightData[] = L"Default", const wchar_t i_szInitializerID[] = L"glorot_uniform") = 0;

		/** ニューラルネットワークにConvolution, Activation, DropOutを行うレイヤーを追加する. */
		virtual Gravisbell::GUID AddNeuralNetworkLayer_CAD(const GUID& i_inputLayerGUID, Vector3D<S32> i_filterSize, U32 i_outputChannelCount, Vector3D<S32> i_stride, Vector3D<S32> i_paddingSize, const wchar_t i_activationType[], Gravisbell::F32 i_dropOutRate, const wchar_t i_szWeightData[] = L"Default", const wchar_t i_szInitializerID[] = L"glorot_uniform") = 0;

		/** ニューラルネットワークにConvolution, BatchNormalization, Activation, DropOutを行うレイヤーを追加する. */
		virtual Gravisbell::GUID AddNeuralNetworkLayer_CBAD(const GUID& i_inputLayerGUID, Vector3D<S32> i_filterSize, U32 i_outputChannelCount, Vector3D<S32> i_stride, Vector3D<S32> i_paddingSize, const wchar_t i_activationType[], Gravisbell::F32 i_dropOutRate, const wchar_t i_szWeightData[] = L"Default", const wchar_t i_szInitializerID[] = L"glorot_uniform") = 0;

		/** ニューラルネットワークにResNetレイヤーを追加する.(ノイズ付き) */
		virtual Gravisbell::GUID AddNeuralNetworkLayer_ResNet(const GUID& i_inputLayerGUID, Vector3D<S32> i_filterSize, F32 i_dropOutRate, U32 i_layerCount, F32 i_noiseVariance=0.0f, const wchar_t i_szWeightData[] = L"Default", const wchar_t i_szInitializerID[] = L"glorot_uniform") = 0;

		/** ニューラルネットワークにResNetレイヤーを追加する. */
		virtual Gravisbell::GUID AddNeuralNetworkLayer_ResNetResize(const GUID& i_inputLayerGUID, Vector3D<S32> i_filterSize, U32 i_outputChannelCount, F32 i_dropOutRate, U32 i_front_layerCount, U32 i_back_layerCount, F32 i_noiseVariance=0.0f, const wchar_t i_szWeightData[] = L"Default", const wchar_t i_szInitializerID[] = L"glorot_uniform") = 0;

		/** ニューラルネットワークにResNetレイヤーを追加する.
			前半/後半に分けずに後半部分だけで処理. */
		virtual Gravisbell::GUID AddNeuralNetworkLayer_ResNetResize_single(const GUID& i_inputLayerGUID, Vector3D<S32> i_filterSize, U32 i_outputChannelCount, F32 i_dropOutRate, U32 i_layerCount, F32 i_noiseVariance=0.0f, const wchar_t i_szWeightData[] = L"Default", const wchar_t i_szInitializerID[] = L"glorot_uniform") = 0;
	};
