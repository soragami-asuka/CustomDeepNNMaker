...
namespace NeuralNetwork {
	...
	/** レイヤー識別コードを取得する.
		...
		@return 成功した場合0 */
	typedef ErrorCode (*FuncGetLayerCode)(GUID& o_layerCode);
	/** バージョンコードを取得する.
	...
	/** レイヤー設定を作成する */
	typedef SettingData::Standard::IData* (*FuncCreateLayerConfig)(void);
	/** レイヤー設定を作成する
		@param i_lpBuffer	読み込みバッファの先頭アドレス.
		@param i_bufferSize	読み込み可能バッファのサイズ.
		@param o_useBufferSize 実際に読み込んだバッファサイズ
		@return	実際に読み取ったバッファサイズ. 失敗した場合は負の値 */
	typedef SettingData::Standard::IData* (*FuncCreateLayerConfigFromBuffer)(const BYTE* i_lpBuffer, int i_bufferSize, int& o_useBufferSize);
