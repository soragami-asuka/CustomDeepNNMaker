...
namespace NeuralNetwork {
	...
	public:
		/** DLLを読み込んで、管理に追加する.
			...
			@return	成功した場合0が返る. */
		virtual Gravisbell::ErrorCode ReadLayerDLL(const wchar_t szFilePath[], GUID& o_addLayerCode) = 0;
		/** DLLを読み込んで、管理に追加する.
			@param szFilePath	読み込むファイルのパス.
			@return	成功した場合0が返る. */
		virtual Gravisbell::ErrorCode ReadLayerDLL(const wchar_t szFilePath[]) = 0;
		...
		/** レイヤーDLLを削除する. */
		virtual Gravisbell::ErrorCode EraseLayerDLL(GUID i_layerCode) = 0;
	};
