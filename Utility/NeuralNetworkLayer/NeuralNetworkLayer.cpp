...
Layer::ILayerData* CreateConvolutionLayer(
	...
	const wchar_t i_szWeightData[], const wchar_t i_szInitializerID[])
{
	...
	// 重みデータ
	{
		SettingData::Standard::IItem_String* pItem = dynamic_cast<SettingData::Standard::IItem_String*>(pConfig->GetItemByID(L"WeightData"));
		pItem->SetValue(i_szWeightData);
	}
	// 初期化方法
	{
...
Layer::ILayerData* CreateMergeMultiplyLayer(
	...
	LayerMergeType i_mergeType)
{
	const Gravisbell::GUID TYPE_CODE(0x7becf785, 0x60d0, 0x42ad, 0x88, 0x57, 0x1e, 0x2c, 0xb2, 0xd5, 0x67, 0x21);
